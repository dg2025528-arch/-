...
with tab1:
    ...
    with col_a:
        fig = px.scatter(
            df_filtered, x="pH Level", y="Species Observed",
            color="SST (°C)", color_continuous_scale="YlOrRd",
            trendline="ols", trendline_color_override=ACCENT,
            labels={"pH Level": "pH 수준", "Species Observed": "관측 종 수",
                    "SST (°C)": "수온(°C)"},
            hover_data={"Location": True}
        )
        fig.update_traces(marker=dict(size=7, opacity=0.7))
        fig = style_fig(fig, "pH ↔ 관측 종 수 관계")
        st.plotly_chart(fig, use_container_width=True)

    with col_b:
        fig = px.box(
            df_filtered, x="백화_한글", y="pH Level",
            color="백화_한글", category_orders={"백화_한글": SEV_ORDER},
            color_discrete_map=SEV_COLORS,
            labels={"백화_한글": "백화 심각도", "pH Level": "pH 수준"}
        )
        fig = style_fig(fig, "백화 심각도별 pH 분포")
    ...
    fig = style_fig(fig, "상관관계 히트맵 (Pearson r)", height=450)
    st.plotly_chart(fig, use_container_width=True)

    valid = df_filtered.dropna(subset=["pH Level", "Species Observed", "Bleaching_Num"])
    r_sp, p_sp = stats.pearsonr(valid["pH Level"], valid["Species Observed"])
    r_bl, p_bl = stats.pearsonr(valid["pH Level"], valid["Bleaching_Num"])
    st.markdown(f"""
    <div class='info-box'>
    📌 <b>통계 해석</b><br>
    • pH ↔ 관측 종 수: r = <b>{r_sp:.3f}</b> (p = {p_sp:.4f})<br>
    • pH ↔ 백화 심각도: r = <b>{r_bl:.3f}</b> (p = {p_bl:.4f})<br>
